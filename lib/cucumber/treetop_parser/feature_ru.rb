...
module Feature
  ...
  def _nt_step_keyword
    ...
    if r1
      ...
      if input.index("Если", index) == index
        r2 = (SyntaxNode).new(input, index...(index + 8))
        @index += 8
      else
        terminal_parse_failure("Если")
        r2 = nil
      end
      if r2
        r0 = r2
      else
        if input.index("То", index) == index
          r3 = (SyntaxNode).new(input, index...(index + 4))
          @index += 4
        else
          terminal_parse_failure("То")
          r3 = nil
        end
        if r3
          r0 = r3
        else
          if input.index("И", index) == index
            r4 = (SyntaxNode).new(input, index...(index + 2))
            @index += 2
          else
            terminal_parse_failure("И")
            r4 = nil
          end
          if r4
            r0 = r4
  ...
  def _nt_scenario_keyword
    ...
    i0, s0 = index, []
    if input.index("Сценарий", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 16))
      @index += 16
    else
      terminal_parse_failure("Сценарий")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_scenario_outline_keyword
    ...
    i0, s0 = index, []
    if input.index("Структура сценария", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 35))
      @index += 35
    else
      terminal_parse_failure("Структура сценария")
      r1 = nil
    end
    s0 << r1
    if r1
  ...
  def _nt_examples_keyword
    ...
    i0, s0 = index, []
    if input.index("Значения", index) == index
      r1 = (SyntaxNode).new(input, index...(index + 16))
      @index += 16
    else
      terminal_parse_failure("Значения")
      r1 = nil
    end
    s0 << r1
    if r1
